# code forked from https://github.com/DariusAf/MesoNet/blob/master/pipeline.py
...
class FaceFinder(Video):
    ...
    def find_coordinates(self, landmark, K=2.2):
        ...
        E = (E1 + E2) / 2
        N = (
            np.mean(landmark["nose_tip"], axis=0) / 2
            + np.mean(landmark["nose_bridge"], axis=0) / 2
        )
    ...
    def find_faces(
        ...
        stop=0,
        skipstep=0,
        no_face_acceleration_threshold=3,
        cut_left=0,
        cut_right=-1,
        use_frameset=False,
        frameset=[],
        ...
        for i in finder_frameset:
            ...
            potential_face_patch = frame[
                potential_location[0] : potential_location[2],
                potential_location[3] : potential_location[1],
            ]
            potential_face_patch_origin = (potential_location[0], potential_location[3])

            reduced_potential_face_patch = zoom(
                potential_face_patch, (resize, resize, 1)
            )
            reduced_face_locations = face_recognition.face_locations(
                reduced_potential_face_patch, model="cnn"
            )

            if len(reduced_face_locations) > 0:
                no_face_acc = 0  # reset the no_face_acceleration mode accumulator

                reduced_face_location = self.pop_largest_location(
                    reduced_face_locations
                )
                face_location = self.upsample_location(
                    reduced_face_location, potential_face_patch_origin, 1 / resize
                )
                ...
                if no_face_acc < no_face_acceleration_threshold:
                    ...
                    face_locations = face_recognition.face_locations(
                        frame, number_of_times_to_upsample=2
                    )
                else:
    ...
    def get_image_slice(img, y0, y1, x0, x1):
        ...
        padded_img = np.pad(
            img, ((padding, padding), (padding, padding), (0, 0)), "reflect"
        )
        return padded_img[
            (padding + y0) : (padding + y1), (padding + x0) : (padding + x1)
        ]
        ...
        while (i < batch_size) and (self.head < self.length):
            if self.head in self.finder.coordinates:
                ...
                batch = np.concatenate(
                    (batch, np.expand_dims(self.resize_patch(patch), axis=0)), axis=0
                )
                i += 1
            self.head += 1
        return batch[1:]
...
def compute_accuracy(classifier, video_file, frame_subsample_count=30):
    """
    Extraction + Prediction over a single video
    """

    # Compute face locations and store them in the face finder
    face_finder = FaceFinder(video_file, load_first_face=False)
